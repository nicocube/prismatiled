...
module.exports = function (uuid) {
    ...
    CenteredTileMap.prototype.sub = function (o) {
        ...
        if ('center' in o && 'dim' in o) {
            ...
            if (this.morph === 'bounded') {
                if (o.tl[0] < o.br[0]) {
                    throw new Error('TopLeft must have vertical coordinate superior to BottomRight. TopLeft: '+o.tl+', BottomRight:'+o.br) 
                }
                if (o.tl[1] > o.br[1]) {
                    throw new Error('TopLeft must have horizontal coordinate inferior to BottomRight. TopLeft: '+o.tl+', BottomRight:'+o.br) 
                }
            } else if (this.morph === 'cylinder') {
                if (o.tl[0] < o.br[0]) {
                    throw new Error('TopLeft must have vertical coordinate superior to BottomRight. TopLeft: '+o.tl+', BottomRight:'+o.br) 
        ...
        for (var i = 0; i < vertiSize; i++) {
            ...
            for (var j = 0; j < horizSize; j++) {
                var lng = (horizStart + j) % this.width
                res[i][j] = this.map[vertiStart+i][lng]
            }              
        }
